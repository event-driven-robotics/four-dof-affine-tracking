...
real_obj_path = "/data/cheezit/cheezit_seq7"
...
index_start =0 
index_start = next(x for x, val in enumerate(y_real_obj_ee_pose) if val > -0.07)
...
index_end = -1
index_end = next(x for x, val in enumerate(time_real_obj_ee_pose) if val > 23)
...
ax2.legend(loc='lower left', bbox_to_anchor= (1.1, 0.38), ncol=1,
            borderaxespad=2, frameon=False)
plt.savefig("cheezit_seq7.png", dpi=300, bbox_inches='tight')